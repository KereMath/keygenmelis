...
void initialize_random_AES_values() {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<bit32> dist(0, UINT32_MAX);

    for (int i = 0; i < 4; i++) {
        AES_plaintext[i] = dist(gen);
        AES_key[i] = dist(gen);
    }
}

void AES_encrypt(bit32 plaintext[], bit32 real_key[], bit32 ciphertext[], int round) {
	bit32 a[4], b[4] = { 0, 0, 0, 0 }, key[4];
	bit32 temp; int j;
	a[0] = plaintext[0]; a[1] = plaintext[1]; a[2] = plaintext[2]; a[3] = plaintext[3];
	key[0] = real_key[0]; 	key[1] = real_key[1]; 	key[2] = real_key[2];	key[3] = real_key[3];
	a[0] ^= key[0];
	a[1] ^= key[1];
	a[2] ^= key[2];
	a[3] ^= key[3];
	for (j = 1; j<round; j++) {
		...
		temp = S[key[3] >> 24] ^ (S[key[3] & 0xff] << 8) ^ (S[(key[3] >> 8) & 0xff] << 16) ^ (S[(key[3] >> 16) & 0xff] << 24);
		temp = temp^key[0] ^ (Rcon[j] << 24);
		key[1] ^= temp;
		key[2] ^= key[1];
		key[3] ^= key[2];
		key[0] = temp;
	...
	b[0] = (S[a[0] >> 24] << 24) ^ (S[(a[1] >> 16) & 0xff] << 16) ^ (S[(a[2] >> 8) & 0xff] << 8) ^ S[a[3] & 0xff];
	b[1] = (S[a[1] >> 24] << 24) ^ (S[(a[2] >> 16) & 0xff] << 16) ^ (S[(a[3] >> 8) & 0xff] << 8) ^ S[a[0] & 0xff];
	b[2] = (S[a[2] >> 24] << 24) ^ (S[(a[3] >> 16) & 0xff] << 16) ^ (S[(a[0] >> 8) & 0xff] << 8) ^ S[a[1] & 0xff];
	b[3] = (S[a[3] >> 24] << 24) ^ (S[(a[0] >> 16) & 0xff] << 16) ^ (S[(a[1] >> 8) & 0xff] << 8) ^ S[a[2] & 0xff];
	temp = S[key[3] >> 24] ^ (S[key[3] & 0xff] << 8) ^ (S[(key[3] >> 8) & 0xff] << 16) ^ (S[(key[3] >> 16) & 0xff] << 24);
	temp = temp^key[0] ^ (Rcon[10] << 24);
	key[1] ^= temp;
	key[2] ^= key[1];
	key[3] ^= key[2];
	key[0] = temp;
...
void AES_write_to_file(FILE *fp, bit32 a[4]) {
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 32; j++) {
			if ((a[i] >> j) & 0x1) 
				fprintf(fp, "1");
...
void generate_AES128_data(int n, int method, int variation, int round) {
	initialize_random_AES_values(); // Rastgele key ve plaintext oluştur

	char str[30000];
	char temp[30000];
	bit32 key[4], plaintext[4], ciphertext[4], t[4];
	FILE *fp=NULL, *fp2=NULL;
	FILE *fptemp;
	char methodname[64];
	for(int i=0; i<4; i++) 
		plaintext[i] = AES_plaintext[i];
	...
	if (method < 8)
		sprintf(outname, "AES128-%s-%d.txt", methodname, variation);
	else
		sprintf(outname, "AES128-%s.txt", methodname);
	
	fp2 = fopen(outname, "w");
	
	if (method != 2 && method < 8) { // CBC does not use input file
		sprintf(inname, "AES128-%s-plaintext.txt", methodname);
		fp = fopen(inname, "w");
		switch (method) {
			case 1:
				gen_inc (128, n, fp);
	...
	for (int i=0; i<n; i++) {
		if (method == 2) {
			...
			for (int j=0; j<4; j++) {
				...
				temp[8] = '\0';
				plaintext[j] = (bit32) strtoul (temp, NULL, 16);
			}
		}
		
		if (method >= 8) {
			AES_encrypt(plaintext, key, ciphertext, round);
			if (method == 8)  {
				for (int j=0; j<4; j++) t[j] = ciphertext[j] ^ plaintext[j];
			}
			else if (method == 9) {
				for (int j=0; j<4; j++) 
					t[j] = ciphertext[j] ^ key[j];
			}
			
			else if (method == 10) {
				for (int j=0; j<4; j++) 
					t[j] = ciphertext[j] ^ plaintext[j] ^ key[j];
			}
			
			AES_write_to_file(fp2, t);
		}
			...
			if (variation == 1 || variation == 2) {
				...
				AES_encrypt(key, plaintext, ciphertext, round);
			}
			
			if (variation == 1 || variation == 3) {
				AES_write_to_file(fp2, ciphertext);
			} else if (variation == 2 || variation == 4) {
				for (int j=0; j<4; j++) 
					t[j] = ciphertext[j] ^ plaintext[j];
				AES_write_to_file(fp2, t);
			}
		}
	}
